...
# https://bitbucket.org/dholth/ed25519ll/raw/37719c56b7b621a98dc694b109ccfca1c946ed65/ed25519ll/djbec.py
...
def decodeint(s_unhexlified):
    return sum(2**i * bit(s_unhexlified,i) for i in range(0,b))

def decodepoint(s_unhexlified):
    y = sum(2**i * bit(s_unhexlified,i) for i in range(0,b-1))
    x = xrecover(y)
    if x & 1 != bit(s_unhexlified,b-1): x = q-x
    P = [x,y]
...
def main():
    for i in range(0,10):
        ...
        print('------ ',i)
        print('sk: ',sk)
        print('pk: ',binascii.hexlify(publickey(sk)))
